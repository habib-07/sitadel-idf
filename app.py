...
sitadel_idf  = pd.read_parquet("C:/Users/habib/Desktop/SITADEL/processed/sitadel_idf_total.parquet")
sitadel_dept = pd.read_parquet("C:/Users/habib/Desktop/SITADEL/processed/sitadel_idf_dept.parquet")
...
dept_opts  = [{"label": f"{dept_labels.get(d,d)} ({d})", "value": str(d)} for d in depts]

sidebar = dbc.Card([
    html.H5("Filtres"),
    html.Hr(),
    html.Label("Annee debut"),
    dcc.Dropdown(id="annee_min", options=annee_opts, value=int(annees[0]),  clearable=False),
    html.Br(),
    html.Label("Annee fin"),
...
app.layout = dbc.Container([
    dbc.Row([html.H2("SITADEL - Construction en Ile-de-France",
...
def update_kpis(amin, amax, depts_sel):
    ...
    return (kpi("Autorisations", f"{aut:,}".replace(",", " ")),
            ...
            kpi("Taux realisation", f"{ratio}%"),
            kpi(f"Evol {last}/{last-1}", f"{sign}{evol}%"))
...
def p_types_evol(amin, amax, depts_sel):
    ...
    fig = px.line(df, x="annee", y="log_aut", color="type_lgt",
                  ...
                  title="Evolution par type",
                  labels={"log_aut":"Logements","annee":"Annee","type_lgt":"Type"})
    fig.update_layout(template="plotly_white")
    return fig
...
def p_dept(amin, amax, depts_sel):
    ...
    fa = px.bar(df, x="annee", y="log_aut", color="dept_lib",
                title="Autorisations par departement",
                labels={"log_aut":"Logements","annee":"Annee","dept_lib":"Dept"})
    fa.update_layout(template="plotly_white")
    fc = px.bar(df, x="annee", y="log_com", color="dept_lib",
                title="Mises en chantier par departement",
                labels={"log_com":"Logements","annee":"Annee","dept_lib":"Dept"})
